...
class CalculatorClass {
  ...
  displayValue(inArr) {
    ...
    resultArray.length < 0
      ? $(".calculated_result_screen").text(resultInput)
      : $(".calculated_result_screen").text(displayInput);
  ...
  addDecimal() {
    ...
    if (this.decimal_flag === false && isNaN(lastInput)) {
    ...
    if (this.decimal_flag === false && !isNaN(lastInput)) {
      this.inputArray[this.inputArray.length - 1] += input;
      // this.inputArray[this.inputArray.length - 1] += lastInput + input;
    }
  ...
  doMath() {
    ...
    const addMinFilter = inArr.filter(val => {
      return val.valueOf() ? val === "+" || val === "-" : "error";
    });
    const { ...am } = addMinFilter;
    let j = 0;

    const multiDivFilter = inArr.filter(val => {
      return val.valueOf() ? val === "*" || val === "/" : "error";
    });
    ...
    while (inArr.length > 1) {
      var oldLength = inArr.length;
      inArr1 = inArr.forEach((val, opIndex) => {
        // let opIndex = inArr.indexOf(val);
        let xPosition = opIndex - 1;
        // let xPosition = inArr.indexOf(inArr[opIndex - 1]);
        let x = parseFloat(inArr[xPosition]);
        let y = parseFloat(inArr[opIndex + 1]);
